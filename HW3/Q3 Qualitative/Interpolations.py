...
for i in range(11):
    ...
    im = Generator(z_)
    save_image(F.upsample(im,scale_factor=6),"./interpolations/GAN/z interpol/image_%d.png" % i, normalize=True)

    # x interpol
    x_ = alpha * im1 + (1 - alpha) * im2

    save_image(F.upsample(x_, scale_factor=6), "./interpolations/GAN/x interpol/image_%d.png" % i, normalize=True)
