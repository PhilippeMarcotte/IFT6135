...
im = Generator(z)
save_image(F.upsample(im,scale_factor=6),"./disentangled/original.png", normalize=True)
...
for index in range(100):
    ...
    save_image(F.upsample(im,scale_factor=6),"./disentangled/image_%d.png" % index, normalize=True)
    # im = im[0].detach().cpu().numpy()
    #
    # plt.imshow(im.transpose((1,2,0)))
    # plt.show()