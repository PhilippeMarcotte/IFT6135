...
def calculate_fid_score(sample_feature_iterator,
    ...
    #start iterators
    sample_feature_iterator.send(None)
    testset_feature_iterator.send(None)
    stack_real = testset_feature_iterator.send(0)
    stack_fake = sample_feature_iterator.send(0)
    # stack outputs before performing calculations
    for i in range(1,999):
        stack_real = np.column_stack((stack_real, testset_feature_iterator.send(i)))
        stack_fake = np.column_stack((stack_fake, sample_feature_iterator.send(i)))

    # mean
    mu_real = np.mean(stack_real,axis=1)
    mu_fake = np.mean(stack_fake, axis=1)

    # covariance
    cov_real = np.cov(stack_real)
    cov_fake = np.cov(stack_fake)# (stack_real - mu_real) * torch.t(stack_real - mu_real) / (stack_real.shape[0] - 1)

    # product of covar + square root ( real part only)
    cov_real_fake = linalg.sqrtm(np.dot(cov_real, cov_fake)).real

    # calculate distance
    dist = linalg.norm((mu_real-mu_fake)) + np.trace(cov_fake) + np.trace(cov_real) - 2*np.trace(cov_real_fake)
    return dist
